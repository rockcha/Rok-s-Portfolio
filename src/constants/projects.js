export const PROJECTS = [
  {
    ...
    key: "spa",
    summary:
      "지금 보고계신 웹 페이지로, 간단한 제 소개와 제작한 프로젝트들을 소개하기 위해 바닐라 JS로 SPA 구조를 모방하여 만들었습니다.",
    link: "#",
    github: "#",
    detail: {
      role: "프론트엔드 개발 (1인)",
      ...
      shortDescription:
        "지금 보고 있는 웹페이지로, 프레임워크 없이 Vanilla JavaScript를 이용해 리액트와 유사한 SPA 라우팅을 직접 구현하기 위한 목적으로 만들었습니다. 부드러운 페이지 전환과 함께 Bootstrap 을 이용해 디자인과 반응형을 신경쓴 자기소개용 웹페이지입니다.",
      mainFeatures: [
        {
      ...
      review:
        "SPA 구조를 직접 구현하면서 라우팅, 렌더링, 이벤트 초기화 흐름을 전체적으로 이해할 수 있었던 프로젝트입니다. 작은 기능도 구조를 어떻게 나누느냐에 따라 유지보수성이 달라진다는 점을 배웠습니다.",
    },
  },
  {
    id: 2,
    ...
    detail: {
      ...
      mainFeatures: [
        {
          ...
          description:
            "spaceship, isLoading, isPending, error를 Zustand 스토어에서 관리하여 UI 컴포넌트는 상태를 구독해서 화면만 책임지도록 분리하고 있습니다. 데이터 조회와 수정 로직을 스토어 액션으로 분리하였으며 Immer를 통한 간결한 불변성 처리를 하고있습니다. ",
      ...
      review:
        "서버 상태와 클라이언트 상태를 나누어 관리하면서 데이터 흐름을 더 명확하게 설계하는 경험을 할 수 있었습니다. 비동기 처리 순서가 사용자 경험에 직접 영향을 준다는 점을 체감했습니다.",
    },
  },
  {
    id: 3,
    ...
    github: "#",
    detail: {
      role: "extension 및 웹 개발",
      period: "2025.01 - 2025.02",
      stack: ["TypeScript", "VS Code API", "Webview", "Node.js"],
      shortDescription:
        "DKMV는 AI가 생성한 코드의 품질을 점수로 정량화하고, 리뷰와 개선 코드까지 함께 제공하여 사용자가 자신의 AI 코딩 실력을 점검하고 웹 대시보드를 이용하여 지속적으로 성장할 수 있게 돕는 서비스입니다.   ",
      mainFeatures: [
        {
      ...
      review:
        "5명의 동료와 함께 협업하며 완성한 결과물로서, 백엔드와 LLM 파트와의 소통을 통해 서비스의 핵심 가치를 극대화하는 API 구조, 데이터 흐름과 UI를 함께 이해하고 논의하면서 수동적으로 정해진 결과를 구현하는 역할에 머무르지 않고 능동적으로 서비스의 만족도를 올리기 위해 노력했습니다. Vs Code 익스텐션 개발은 특히 처음 경험해보는 영역이었고, 웹과는 다른 실행 환경과 제약 조건을 이해하는 과정이 쉽지 않았습니다. 하지만, 최종적으로 한컴 AI 아카데미 2기 최종 프로젝트에서 최우수팀으로 선정되면서 가장 재미있는 협업 경험이자 프로젝트 경험으로 남게 되었습니다. ",
    },
  },
  {
    id: 4,
    title: "4️⃣ 하루, 담 - 멀티기능 일정관리 서비스 ",
    logoName: "하루, 담",
    key: "harudam",
    summary:
      "할일/일정/메모/음악/일기까지 관리하는 멀티기능 일정관리 서비스입니다.",
    link: "#",
    github: "#",
    detail: {
      role: "프론트엔드 개발",
      period: "2025.06 - 2025.07",
      stack: ["Supabase", "JavaScript", "HTML/CSS", "Responsive Web"],
      shortDescription:
        "하루 하루의 내용을 이 웹 페이지에 담아낸다 라는 의미로 만든 서비스이며, Supabase에 오늘의 할 일, 일정, 나의 음악, 메모, 일기 데이터를 저장하고 조회/추가/수정/삭제(CRUD)할 수 있는 멀티기능 일정 페이지이며, 반응형 레이아웃과 깔끔한 디자인 완성도에 집중한 프로젝트입니다. 지인들의 사용후기로 지속적인 피드백을 받아 기능의 다양성과 ux를 개선하고 있습니다. ",
      mainFeatures: [
        {
          title: "오늘의 할 일 낙관적 업데이트",
          description:
            "사용자가 할 일을 체크하거나 수정하면 서버 응답을 기다리지 않고 UI를 즉시 갱신하도록 구현했습니다. 이후 비동기 요청 결과에 맞춰 데이터를 동기화하여, 느린 네트워크 환경에서도 빠르고 자연스러운 사용 경험을 제공합니다. 요청 실패시 원본으로 복구하는 작업까지 안전하게 구현하였습니다.",
      ...
      troubleShooting: [
        {
          ...
          problem:
            "홈 대시보드의 여러 섹션이 초기 진입 시 동시에 데이터를 요청하면서, 사용자가 보지 않는 접힌 영역까지 불필요하게 네트워크 요청이 발생하였습니다. 현재는 섹션이 여러개가 아니지만, 앞으로 섹션이 추가될때마다, 서버의 데이터가 늘어날때마다 성능 저하를 피할 수 없다고 판단하였습니다. ",
          solution:
      ...
      review:
        "일정과 메모 데이터를 직접 조작하는 CRUD 흐름을 구현하면서 사용자가 자주 쓰는 화면일수록 단순하고 안정적인 인터랙션이 중요하다는 점을 배웠습니다.",
    },
  },
];
